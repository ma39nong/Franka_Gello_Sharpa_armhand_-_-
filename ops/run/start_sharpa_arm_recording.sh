#!/usr/bin/env bash
# Record the running dual-FR3 + dual-Sharpa ROS graph without opening another
# libfranka/FCI connection. Three-camera capture is opt-in with --with-cameras.
set -euo pipefail
...
usage() {
  cat >&2 <<'EOF'
Usage: ./ops/run/start_sharpa_arm_recording.sh [--with-cameras] [--without-hands]
...
unless --without-hands is used, the CycloneDDS Sharpa wrapper first. This
...
readonly -a TOPICS EXPECTED_TYPES
...
if [[ "$INCLUDE_HANDS" == true ]]; then
  bag_group=gello_sharpa
elif [[ "$WITH_CAMERAS" == true ]]; then
  bag_group=gello_arms_camera
else
  bag_group=gello_arms
fi
...
collector_name="sharpa-data-collection-$$"
docker compose run -d --name "$collector_name" --no-deps \
  --user "$(id -u):$(id -g)" \
  -e HOME=/tmp \
  -e ORBBEC_SDK_LICENSE_ACCEPTED="${ORBBEC_SDK_LICENSE_ACCEPTED:-NO}" \
  -e COLLECTION_RECORD_CONFIG=/workspace/franka_upper_body_teleop/data_collection/config/record_gello_sharpa.yaml \
...
stop_collector() {
  if [[ "$stop_requested" == false ]]; then
    stop_requested=true
    echo "Stopping the Sharpa collector after active bag finalization..."
